        ...
        <>
          ...
          >
              ...
              <Controls.Grid item sx={{ textAlign: "center" }} xs={12}>
                ...
                {/* <Controls.Grid item sx={{display:{xs:"block",sm:"none"}}}>
              {products.length > 0 && products.map((item, index) => (
                <>

                  <Controls.Grid item xs={12} sx={{ display: "flex", margin: "auto", padding:"10px"}} key={index} gap={15}>
                    <Controls.Card sx={{ display: "flex", backgroundColor:"transparent"}} gap={2}>
                      <Controls.CardMedia component="img" src={item.image} width="100%" height="100%" sx={{ height: "100px",width:"100px" }} />
                      
                        <Controls.Typography
                          variant='caption1'
                          sx={{ fontSize: '15px', fontWeight: 'bold', fontFamily:"Quicksand"  }}
                        >
                          {item.name}
                        </Controls.Typography>
                        <Controls.Grid container justifyContent="space-between">
                          <Controls.Grid item>
                            <Controls.Typography variant='h6' sx={{ fontSize: '18px', fontWeight: "medium" }}>
                              Price: Rs
                               {item.price}
                          ...
                          <Controls.Grid item>
                            <Controls.Typography variant='caption1' sx={{ fontSize: '18px', fontWeight: "medium" ,marginLeft:"50px"}}>
                            {item.price * item.quantity}
                              Qty:{item.quantity} {item.quantity > 1 ? 'pieces' : 'piece'}
                            </Controls.Typography>
                          </Controls.Grid>
                        </Controls.Grid>
                        <Controls.Grid container alignItems="center" gap={0.5}  >
                          <Controls.Grid item onClick={() => handleDecrement(item._id)}>
                            <Controls.Button variant="outlined" sx={{padding:1}}>
                              <Controls.RemoveIcon fontSize='small'/>
                            </Controls.Button>
                          </Controls.Grid>
                        ...
                        <Controls.Grid item sx={{alignItem:"center"}} mt={2}>
                          <Controls.Button
                            variant='contained'
